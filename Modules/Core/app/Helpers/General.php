...
function loadSettings(): void
{
    ...
    \Modules\System\Actions\Tenant\BootTenantAction::configureTenantFilesystemBySubdomain();
    ...
    $currency = Currency::find($all_settings['currency_id'] ?? 1);
...
{
    // Store current tenant connection state if we're on one
    ...
    try {
        ...
        if ($wasOnTenant && $currentFacility) {
            TenantDatabaseService::connectToFacility($currentFacility);
        } elseif ($wasOnTenant && $currentConnection === 'tenant') {
            // Fallback: restore connection name if facility is not available
