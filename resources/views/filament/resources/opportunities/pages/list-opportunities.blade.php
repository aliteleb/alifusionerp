                    ...
                    <div class="relative">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 max-h-60 overflow-hidden">
                            ...
                            <div class="overflow-y-auto max-h-48">
                                <button type="button" @click="$wire.set('branchId', ''); $wire.call('loadKanbanOpportunities'); open = false" class="w-full text-left rtl:text-right px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="$wire.branchId === '' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                                    {{ __('All Branches') }}
                                </button>
                                <template x-for="branch in branches.filter(b => b.name.toLowerCase().includes(search.toLowerCase()))" :key="branch.id">
                                    <button type="button" @click="$wire.set('branchId', branch.id); $wire.call('loadKanbanOpportunities'); open = false" class="w-full text-left rtl:text-right px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="$wire.branchId == branch.id ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                ...
                <div x-data="{ open: false, search: '', projects: @js(\App\Core\Models\Project::active()->get()->sortBy('name')->map(fn($p) => ['id' => $p->id, 'name' => $p->name])->values()) }">
                    <div class="relative">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 max-h-60 overflow-hidden">
                            ...
                            <div class="overflow-y-auto max-h-48">
                                <button type="button" @click="$wire.set('projectId', null); $wire.call('loadKanbanOpportunities'); open = false" class="w-full text-left rtl:text-right px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="!$wire.projectId ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                                    {{ __('All Projects') }}
                                </button>
                                <template x-for="project in projects.filter(p => p.name.toLowerCase().includes(search.toLowerCase()))" :key="project.id">
                                    <button type="button" @click="$wire.set('projectId', project.id); $wire.call('loadKanbanOpportunities'); open = false" class="w-full text-left rtl:text-right px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="$wire.projectId == project.id ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                        ...
                        <div class="space-y-2 min-h-[500px]" id="stage-{{ $stage->value }}">
                            @foreach($stageOpportunities as $opportunity)
                                <div
                                    ...
                                    x-init="
                                        const opportunityCard = $el;
                                        if (window.Livewire) {
                                            Livewire.on('opportunity-priority-updated', (data) => {
                                                if (data.opportunityId == {{ $opportunity->id }}) {
                                                    opportunityCard.classList.remove('saving-opportunity');
                                                    opportunityCard.style.borderColor = '';
                                                    opportunityCard.style.boxShadow = '';
                                                    
                                                    // Update priority badge
                                                    if (data.priority !== undefined) {
                                                        const priorityBadge = document.querySelector(`[data-opportunity-priority-badge='{{ $opportunity->id }}']`);
                                                        if (priorityBadge) {
                                                            const priorities = {
                                                                ...
                                                                'critical': { label: '{{ __('Critical') }}', colorClass: 'bg-danger-100 text-danger-700 dark:bg-danger-900 dark:text-danger-300' }
                                                            };
                                    ...
                                    <div class="absolute top-1.5 left-1 right-1 z-10 flex items-center justify-between gap-0.5">
                                        ...
                                        @if($opportunity->forecast_close_date)
                                            <span class="text-xs text-gray-600 dark:text-gray-400 font-medium">
                                                {{ $opportunity->forecast_close_date->translatedFormat('M d, Y') }}
                                            </span>
                                        @else
                                            <span></span>
                                        @endif
                                        ...
                                        @if($opportunity->priority)
                                            ...
                                            @endphp
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium {{ $priorityBadgeClass }}" data-opportunity-priority-badge="{{ $opportunity->id }}">
                                        ...
                                        <div class="space-y-1 text-xs" x-show="expanded" x-transition>
                                            ...
                                            @if($opportunity->updated_at)
                                                <div class="flex items-center gap-1 text-gray-500 dark:text-gray-500 pt-1 border-t border-gray-200 dark:border-gray-700">
                                                    ...
                                                    <span class="truncate">{{ __('Last update: :date', ['date' => $opportunity->updated_at->translatedFormat('M d, Y H:i')]) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
