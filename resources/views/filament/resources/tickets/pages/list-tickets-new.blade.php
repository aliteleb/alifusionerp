                    ...
                    <div class="relative">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 max-h-60 overflow-hidden">
                            ...
                            <div class="overflow-y-auto max-h-48">
                                <button type="button" @click="$wire.set('branchId', ''); $wire.call('loadKanbanTickets'); open = false" class="w-full text-left px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="$wire.branchId === '' ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                                    {{ __('All Branches') }}
                                </button>
                                <template x-for="branch in branches.filter(b => b.name.toLowerCase().includes(search.toLowerCase()))" :key="branch.id">
                                    <button type="button" @click="$wire.set('branchId', branch.id); $wire.call('loadKanbanTickets'); open = false" class="w-full text-left px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="$wire.branchId == branch.id ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                ...
                <div x-data="{ open: false, search: '', projects: @js(\App\Core\Models\Project::active()->get()->sortBy('name')->map(fn($p) => ['id' => $p->id, 'name' => $p->name])->values()) }">
                    <div class="relative">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 max-h-60 overflow-hidden">
                            ...
                            <div class="overflow-y-auto max-h-48">
                                <button type="button" @click="$wire.set('projectId', null); $wire.call('loadKanbanTickets'); open = false" class="w-full text-left px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="!$wire.projectId ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                                    {{ __('All Projects') }}
                                </button>
                                <template x-for="project in projects.filter(p => p.name.toLowerCase().includes(search.toLowerCase()))" :key="project.id">
                                    <button type="button" @click="$wire.set('projectId', project.id); $wire.call('loadKanbanTickets'); open = false" class="w-full text-left px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="$wire.projectId == project.id ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                    ...
                    <div class="relative">
                        <x-filament::input.wrapper>
                            <div class="relative">
                                ...
                                <button type="button" @click="open = !open" class="fi-input block w-full rounded-lg border-gray-300 shadow-sm transition duration-75 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:focus:border-primary-500 sm:text-sm text-left pl-10 pr-10 py-1.5">
                                    <span :class="$wire.limit === 100 ? 'text-gray-400 dark:text-gray-500' : ''" class="truncate block text-left" x-text="$wire.limit === 100 ? '{{ __('Limit') }}' : (limits.find(l => l.value == $wire.limit)?.label || $wire.limit)"></span>
                                </button>
                            </div>
                        </x-filament::input.wrapper>
                        <div x-show="open" @click.away="open = false" x-transition class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 max-h-60 overflow-hidden">
                            <div class="overflow-y-auto max-h-60">
                                <template x-for="limit in limits" :key="limit.value">
                                    <button type="button" @click="$wire.set('limit', limit.value); $wire.call('updatedLimit', limit.value); open = false" class="w-full text-left px-2.5 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm" :class="$wire.limit == limit.value ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-gray-700 dark:text-gray-300'">
                                        ...
                                        if (window.Livewire) {
                                            Livewire.on('ticket-priority-updated', (data) => {
                                                if (data.ticketId == {{ $ticket->id }}) {
                                                    $el.classList.remove('saving-ticket');
